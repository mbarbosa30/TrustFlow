...
**Microcredit Lending System (IN DEVELOPMENT)**: Community-opt-in USDC microlending with trust-based eligibility. Core infrastructure complete (Tasks 1-4): Database schema extended with 6 tables (loan, installment, subsidy_ledger, assist, guarantee, trust_event). Lending policy system with configurable parameters (loan amounts: $160-$800 USDC, tenors: 6-12 months, 40% APR, GHI ≥60 requirement). Loan eligibility engine validates community health + Sybil-resistance checks (min-cut ≥2, isAccepted). Installment schedule generator uses standard amortization formula with month-end date normalization. Transaction-safe loan creation via `db.transaction()`. **Limitation**: Vertex-disjoint paths and per-seed share metrics not stored per-user - eligibility relies on `isAccepted` flag which already implies these thresholds passed during scoring. Lending policy thresholds MUST match scoring policy thresholds. Remaining tasks: subsidy systems (Interest Buy-Down, Repay-Assist, Vouchers, First-Loss Guarantee), payment processing, trust event integration, UI components, economic dashboard, exchange rate service.
