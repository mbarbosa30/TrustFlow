...
**Microcredit Lending System (MVP COMPLETE)**: Community-opt-in USDC microlending with trust-based eligibility and supporter subsidy systems. **Core Infrastructure (Tasks 1-11, 15-16 COMPLETE)**: Database schema extended with 6 tables (loan, installment, subsidy_ledger, assist, guarantee, trust_event). Lending policy system with configurable parameters (loan amounts: $160-$800 USDC, tenors: 6-12 months, 40% APR, GHI ≥60 requirement). Loan eligibility engine validates community health + Sybil-resistance checks (min-cut ≥2, isAccepted). Installment schedule generator uses standard amortization formula with month-end date normalization. Transaction-safe loan creation via `db.transaction()`. **Subsidy Systems**: Interest Buy-Down (IBD) allows supporters to pledge USDC reducing borrower interest rates. Repay-Assist (RA) enables supporters to cover late installments with 6% premium repayment waterfall (exact match validation prevents under/overpayments). Interest Vouchers waive N months of interest. First-Loss Guarantee creates default waterfall: Guarantee → RA claims → Pool loss. **Payment Processing**: Installment payments with grace period (5 days), late marking (>7 days), default trigger (>60 days), FLG waterfall execution. **Trust Integration**: Conservative delta recording (+0.02 on-time, -0.05 late, -0.15 default, ±0.03 assist) with epoch-level caps (±0.10 per user per epoch) prevents gaming. Post-scoring delta application system ready for epoch workflow integration. **Exchange Rate Service**: Mock ARS/USD conversion with formatting utilities, ready for real-time API integration. **Borrower Credit UI**: /credit route with eligibility check, loan application (amount × tenor selector), active loan card (progress bar, payment interface), transaction history. API endpoints operational with correct route ordering. **Security Status**: Payer authentication requires EIP-712 signature verification for production (documented as TODO). **Limitation**: Vertex-disjoint paths and per-seed share metrics not stored per-user - eligibility relies on `isAccepted` flag which already implies these thresholds passed during scoring. Lending policy thresholds MUST match scoring policy thresholds. **Remaining Phase 2 Tasks**: Supporter subsidy UIs (Task 12), community economic dashboard (Task 13), lending policy admin UI (Task 14), wallet signature verification.
